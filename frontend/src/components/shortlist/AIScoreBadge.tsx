...
export function AIScoreBadge({
  ...
  return (
    <div className={cn('flex flex-col items-center gap-1', className)}>
      ...
      >
        <span className={cn('font-mono font-bold', sizeClasses[size].grade, colors.text)}>{grade}</span>
        <span className={cn('font-mono font-semibold', sizeClasses[size].score, colors.scoreText)}>
          {score}
        </span>
      </div>
      {showLabel && <p className="text-xs text-text-tertiary font-mono font-medium tracking-wide uppercase">AI Score</p>}
    </div>
  )
}
