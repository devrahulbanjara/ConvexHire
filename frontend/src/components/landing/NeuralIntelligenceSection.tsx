...
const NEURAL_COLORS = {
  ...
  gradients: {
    forward:
      'radial-gradient(ellipse at center, rgba(59,130,246,0.12) 0%, rgba(96,165,250,0.08) 35%, rgba(147,197,253,0.04) 70%, rgba(249,250,251,0) 100%)',
    backward:
      'radial-gradient(ellipse at center, rgba(236,72,153,0.12) 0%, rgba(244,114,182,0.08) 35%, rgba(249,168,212,0.04) 70%, rgba(249,250,251,0) 100%)',
    neutral:
      'radial-gradient(ellipse at center, rgba(139,92,246,0.08) 0%, rgba(167,139,250,0.06) 35%, rgba(196,181,253,0.03) 70%, rgba(249,250,251,0) 100%)',
  },
}
...
export function NeuralIntelligenceSection() {
  ...
  const updateSignals = useCallback((updater: Signal[] | ((prev: Signal[]) => Signal[])) => {
    setSignals(prev => {
  ...
  }, [isInView, prefersReducedMotion, neurons, animationPhase, updateSignals])
                ...
                >
                    ...
                    signals.map(signal => {
                      ...
                      return (
                        <g key={signal.id}>
                          ...
                          {/* Bright core */}
                          <circle cx={x} cy={y} r={currentSize * 0.6} fill="white" opacity="0.9" />
                  ...
                  {neurons.map((neuron, i) => {
                    ...
                    const rgbaFill = rgb
                      ...
                      : layerColor
                    const rgbaGlow = rgb
                      ? `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${isHovered ? 0.8 : glowIntensity * 0.6})`
                      : layerColor
