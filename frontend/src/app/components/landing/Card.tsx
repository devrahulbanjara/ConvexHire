...
export default function Card({
  ...
  return (
    ...
    >
      ...
      {icon && (
        <div className="flex items-center justify-center w-12 h-12 p-3 bg-primary-100 dark:bg-primary-950/30 rounded-[6px] mb-7">
          <Image src={icon} alt={iconAlt} width={24} height={24} className="object-contain" />
        </div>
      )}

      <h3 className="text-xl font-bold text-[var(--color-primary-black)] mb-4">{title}</h3>

      <p className="text-base leading-6 text-[var(--color-gray-600)]">{description}</p>
    </div>
  )
}
