...
def get_my_applications(request: Request, db: Session = Depends(get_db)):
...
def get_tracking_board(request: Request, db: Session = Depends(get_db)):
    user_id = get_current_user_id(request)
...
def get_application_stats(request: Request, db: Session = Depends(get_db)):
    user_id = get_current_user_id(request)
...
def create_application(
    app_data: CreateApplicationRequest, request: Request, db: Session = Depends(get_db)
):
    user_id = get_current_user_id(request)
