...
logger.add(OUTPUT_DIR / "ats_screening.log", rotation="10 MB", level="INFO")


def main():
    logger.info("ATS Resume Screening System started")
    ...
    try:
        ...
        if final_report:
            ...
            logger.success(f"Workflow completed: {summary['total_candidates']} candidates, {summary['shortlisted_count']} shortlisted, {summary['rejected_count']} rejected")

        logger.info(f"Output files generated in: {OUTPUT_DIR}")

    except FileNotFoundError as e:
        logger.error(f"File not found: {e}")
        logger.error("Please ensure you have created 'resumes' directory with job_description.txt and resume files")
        return 1
