...
class ResumeService:
    ...
    def create_resume(self, user_id: str, resume_data: dict) -> ResumeResponse:
        ...
        contact_location = resume_data.get("contact_location") or self._format_location(
            profile.location_city, profile.location_country
        )
        custom_summary = (
            resume_data.get("custom_summary") or profile.professional_summary
        )
    ...
    def add_experience_to_resume(
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="Experience already added to this resume"
            )
    ...
    def update_experience_in_resume(
        ...
        if experience_data.get("start_date"):
            resume_experience.start_date = datetime.strptime(
                experience_data["start_date"], "%Y-%m-%d"
            ).date()
        if experience_data.get("end_date"):
            resume_experience.end_date = datetime.strptime(
    ...
    def add_education_to_resume(
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="Education already added to this resume"
            )
    ...
    def update_education_in_resume(
        ...
        if education_data.get("start_date"):
            resume_education.start_date = datetime.strptime(
                education_data["start_date"], "%Y-%m-%d"
            ).date()
        if education_data.get("end_date"):
            resume_education.end_date = datetime.strptime(
    ...
    def add_certification_to_resume(
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="Certification already added to this resume"
            )
    ...
    def update_certification_in_resume(
        ...
        if not resume_certification:
            raise HTTPException(
                status_code=404, detail="Resume certification not found"
            )
        ...
        if certification_data.get("issue_date"):
            resume_certification.issue_date = datetime.strptime(
                certification_data["issue_date"], "%Y-%m-%d"
            ).date()
        if certification_data.get("expiration_date"):
            resume_certification.expiration_date = datetime.strptime(
    ...
    def add_skill_to_resume(
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="Skill already added to this resume"
            )
