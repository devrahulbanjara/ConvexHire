...
class WorkExperience(BaseModel):
    ...
    responsibilities: list[str] = Field(
        default_factory=list, description="List of key duties"
    )
...
class Education(BaseModel):
    ...
    year: int | None = Field(None, description="Year of graduation if available")
...
class ResumeStructured(BaseModel):
    ...
    years_experience: float = Field(
        ..., description="Total sum of professional experience in years"
    )
    projects: list[Project] = Field(
        default_factory=list, description="Relevant projects"
    )
